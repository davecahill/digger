...
ifeq ($(ARCH),"MINGW")
RCFLAGS	+= -O -mno-cygwin -DMINGW -Dmain=SDL_main -I../zlib -I../SDL-1.1.2/include/SDL
...
RCFLAGS	+= -O -DFREEBSD $(shell sdl-config --cflags)
...
ifeq ($(ARCH),"LINUX")
OBJS	+= fbsd_sup.o	# strup()
RCFLAGS	+= -O -DLINUX $(shell sdl-config --cflags)
LIBS	+= $(shell sdl-config --libs) -lz
ESUFFIX	=
endif
...
RCFLAGS	+= -O -DFooOS	# insert here additional compiler flags which required to find include files, trigger os-specific compiler behaviour etc.
...
OBJS	+=		# insert here the names of the files which contains various missing functions like strup() on Linux and FreeBSD
RCFLAGS	+= -DJS -O1	# insert here additional compiler flags which required to find include files, trigger os-specific compiler behaviour etc.
LIBS	+= 		# insert here libs required to compile like zlib, SDL etc
ESUFFIX	=		# insert here suffix of the executable on your platform if any (like ".exe" on Win32)
endif
